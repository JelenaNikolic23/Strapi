...
export function Detailed7P(props) {
  ...
  useEffect(() => {
    ...
    if (evaluationId) {
      axios
        .get(`${serverip}/api/7ps/${evaluationId}`, request_config())
        .then(evaluation => {
          ...
          if (parseInt(frontline_id) === evaluation.data.user.frontline_id) {
            axios
              .get(
                `${serverip}/api/7ps/updateSeen/${evaluationId}`,
                request_config()
              )
              .then(() => {
  ...
  const onSubmit = e => {
    ...
    axios
      .post(
        `${serverip}/api/7ps/feedbackReplay/${evaluationId}`,
        body,
        request_config()
      )
      .then(res => {
          ...
          <Card>
            <CardHeader color="primary">
              <div style={{ marginBottom: "12px" }}>
                <h4 className={classes.cardTitleWhite}>7P Evaluacija</h4>
              </div>
              <GridContainer>
...
export default withStyles(styles)(Detailed7P);
