...
function Evaluations7PTable({ frontline_id, tooltip }) {
  ...
  useEffect(() => {
    ...
    axios
        .post(
            `${serverip}/api/7ps/datepicker/user/${frontline_id}`,
            evalDateBody,
            request_config()
          )
          .then(newEvaluations => {
            setEvaluationData(newEvaluations.data);
          })
          .catch(err => {
            console.log(err);
          });
  ...
  const EvalDetails = evaluation => {
    return [
      <Link
        to={`/admin/reports/people/7P/${evaluation.user.frontline_id}/${
          evaluation._id
        }`}
        style={{
          color: "#033E8C",
...
export default Evaluations7PTable;
